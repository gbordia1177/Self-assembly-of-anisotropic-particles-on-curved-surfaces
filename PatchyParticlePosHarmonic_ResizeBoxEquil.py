...
equil_file = "./equilfiles/FixPhi/PosHarmonic2DWavePatchyParticles_N_" + str(N) + "_phi_" + str(phi) + "_P_" + str(P) + "_epsP_" + str(eps_p) + "_k_" + str(k) + "_delh_" + str(delh) + "_q_"+str(period)+  "_even_" + str(even) + "_.gsd"
...
unit_cell = np.sqrt((np.pi*(radius**2)  )/phi) #unit cell length dimension for square lattice

ncopy = int(np.sqrt(N)) #number of unit cell copies in each dimension. assuming box is square
#The length of the box when assuming we have a flat surface
L = ncopy*unit_cell #length of box

#q= 2*np.pi/(boxlength) * period
#compute the modified box length that conserves area from the flat surface and accounts for the wavy surface.
Lprime = fsolve(func2, L, args=(delh, period, L))[0]
unit_cell_prime = Lprime/ncopy

print('L = '+str(L))
print('Lprime = '+str(Lprime))

#print(ncopy)
#print(L)
N = ncopy*ncopy #recompute total number of core particles
...
harmonic.params['C'] = dict(k = k/2, delh=delh, q= 2*np.pi/(boxlength) * period, dim=2)
...
harmonic.params['C']= dict(k=k*4, delh=delh, q= 2*np.pi/(boxlength) * period, dim=2)
...
harmonic.params['C']= dict(k=k*8, delh=delh, q= 2*np.pi/(boxlength) * period, dim=2)
#gsd_writer_restart = hoomd.write.GSD(filename = restart_file, trigger = hoomd.trigger.Periodic(int(num_steps)), mode='wb', filter=hoomd.filter.All(), dynamic=['property'])
#sim.operations.writers.append(gsd_writer_restart)
sim.run(num_equil_steps)

harmonic.params['C']= dict(k=k*16, delh=delh, q= 2*np.pi/(boxlength) * period, dim=2)
#gsd_writer_restart = hoomd.write.GSD(filename = restart_file, trigger = hoomd.trigger.Periodic(int(num_steps)), mode='wb', filter=hoomd.filter.All(), dynamic=['property'])
#sim.operations.writers.append(gsd_writer_restart)
sim.run(num_equil_steps)

harmonic.params['C']= dict(k=k*25, delh=delh, q= 2*np.pi/(boxlength) * period, dim=2)
#gsd_writer_restart = hoomd.write.GSD(filename = restart_file, trigger = hoomd.trigger.Periodic(int(num_steps)), mode='wb', filter=hoomd.filter.All(), dynamic=['property'])
#sim.operations.writers.append(gsd_writer_restart)
sim.run(num_equil_steps)

harmonic.params['C']= dict(k=k*50, delh=delh, q= 2*np.pi/(boxlength) * period, dim=2)
sim.run(num_equil_steps)
